<head>
    ...
    *[x-apple-data-detectors],
    ...
    <td align="center" style="margin: 0; padding-left: 10px; padding-right: 10px; padding-top: 15px; padding-bottom: 20px;">
    <p style="margin: 0; -webkit-text-size-adjust: none; -ms-text-size-adjust: none; mso-line-height-rule: exactly; font-family: arial, 'helvetica neue', helvetica, sans-serif; line-height: 21px; color: #666666; font-size: 14px;">
    Đã Vận Chuyển @if ($order->status >= 2) <br /> {{ $order->date_ship }} @endif
    </p>
    </td>
    </tr>
    </tbody>
    ...
    <td align="center" style="margin: 0; padding-left: 10px; padding-right: 10px; padding-top: 15px; padding-bottom: 20px;">
    <p style="margin: 0; -webkit-text-size-adjust: none; -ms-text-size-adjust: none; mso-line-height-rule: exactly; font-family: arial, 'helvetica neue', helvetica, sans-serif; line-height: 21px; color: #666666; font-size: 14px;">
    Giao Hàng Thành Công @if ($order->status == 3) <br /> {{ $order->date_s }} @endif
    </p>
    </td>
    </tr>
    </tbody>
    ...
    @if ($type != 4) {{ $subject }}, Chuẩn bị chiến thôi giáo sư! @else Thật đáng tiếc khi đơn hàng của quý khách đã bị huỷ :(( , Mong quý khách vẫn ủng hộ 2NITE SHOP @endif
    </p>
    <p style="mso-line-height-rule: exactly; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, 'Karla'; font-size: 16px; line-height: 26px; font-weight: 400; color: #666363; margin: 13px 0;" align="center">{{ $text }}</p>
    </th>
    </tr>
    ...
    <span data-key="1468270_order_number">ID đơn hàng: </span>{{ $order->id }}
    </h2>
    ...
    {{-- item --}} @foreach ( $cart as $item ) @php $prd = App\Models\Products::where('id', '=' , $item->id)->first(); @endphp
    ...
    <a href="{{ route('detail_product', ['slug'=>$prd->slug]) }}" target="_blank" style="color: #666363; text-decoration: none !important; text-underline: none; word-wrap: break-word; text-align: left !important; font-weight: bold;">
    {{ $prd->name }}
    </a>
    <br />
    ...
    {{ App\Models\Insurance::where('id','=' ,$item->options->ins)->first()->group == 1 ? "Chế độ bảo hành:":"Phụ kiệm mua kèm" }} {{ App\Models\Insurance::where('id','=',$item->options->ins)->first()->name}}
    </span>
    @endif
    </p>
    </th>
    ...
    {{ $order->name }}<br />
    {{ $order->phone }} <br />
    @if($order->note!= null) {{$order->note}} <br />
    @endif
    <a href="mailto:" style="color: #ecba78; text-decoration: none !important; text-underline: none; word-wrap: break-word;" target="_blank">{{ $order->email }}</a>
    </p>
    </th>
    </tr>
    </tbody>
    ...
    Nếu khách hàng cần sự giúp đỡ hãy liên hệ với email: vaone6v2@gmail.com hoặc GỌI: {{getVal('hotline')->value }} :)
    </p>
    </th>
    </tr>
