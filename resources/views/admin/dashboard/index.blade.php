...
<script src="{{ $file->ver('admin/app/js/dashboard.js') }}">
</script>
...
<input type="hidden" name="" id="url__ajax--task" value="{{ route('todos') }}">
<input type="hidden" name="" id="url__ajax--calender" value="{{ route('fullcalender_ajax') }}">
<input type="hidden" name="" id="url__calender" value="{{ route('fullcalender') }}">
<input type="hidden" name="" id="url__chart" value="{{ route('load__chart') }}">
            ...
            <div class="col-3 stat__item --users">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="card__bot">
                            <span>{{ crf_2($stats_users) }} Users</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-3 stat__item --orders">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="card__bot">
                            <span>{{ crf_2($stats_order) }} Đơn</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-3 stat__item --month">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="card__bot">
                            <span>{{ crf($stats_revenueToday) }}</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-3 stat__item --year">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="card__bot">
                            <span>{{ crf($stats_revenueMonth) }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-3 stat__item --products">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="card__bot">
                            <span>{{ crf_2($stats_product) }} Sản Phẩm</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-3 stat__item --blogs pl-0">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="card__bot">
                            <span>{{ crf_2($stats_blog) }} Bài Viết</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-3 stat__item --comments">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="card__bot">
                            <span>{{ crf_2($stats_comment) }} Bình Luận</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card --main">
                    ...
                    <div class="card-body">
                        <div class="row mx-0 card__users">
                            @foreach (App\Models\User::where('role_id' , '>' , 3)->where('id' , '!=' ,
                            Auth::id())->orderBy('id' , 'DESC') ->limit(8) -> get() as $user )
                        ...
                        <table class="table table-borderless">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $blog->title }}</td>
                                    <td>{{ $blog->author }}</td>
                                    <td>{{ $carbon->create($blog->created_at)->diffForHumans($carbon->now()) }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                        ...
                        <table class="table table-borderless">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $product->name }}</td>
                                    <td>
                                        <img src="{{ asset($product->main_img) }}" width="150px" height="150px"
                ...
                <div class="card">
                    <div class="card-header text-center">
                        ...
                        <div class="task-pages">
                            {!! navi_ajax_page($number_page , $page , 'pagination-sm' , 'justify-content-end' , 'mt-2')
                            !!}
                        </div>
                    </div>
                    <div class="card-body task">
                        ...
                        <div
                            class="task__item task-{{ $task -> id }} @if ($task -> done == 1) task__item--done @endif @if ($task -> done == 0 && $task->time_end <= $now ) task__item--done @endif ">
                            <div class="va-checkbox">
                                <input type="checkbox" name="" id="tk-{{ $task -> id }}" class="task__item--check"
                                    data-id="{{ $task -> id }}" @if ($task -> done == 1) checked @endif>
                                <label for="tk-{{ $task -> id }}">{{ $task -> name }} @if ($task
                                    -> done == 1)
                                    <span class="badge badge-success">Hoàn Thành</span>
                                    @else
                                    @if ($task->time_end > $now)
                                    <span class="badge badge-warning">Còn {{ timeDiff($now , $task->time_end ) }}</span>
                                    @else
                    ...
                    <div class="card-footer">
                        <div class="d-flex justify-content-center align-items-center">
                            ...
                            <a href="" class="ml-4 see-all btn_reload--task"><i class="fas fa-sync-alt pr-2"></i>Làm
                                Mới</a>
                            {{-- <a href="" class="ml-4 see-all btn_reload--task "><span
