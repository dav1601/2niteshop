...
@section('content')
    <div id="content__dashboard">
        <div class="row mx-0">
            <div class="stat col-12 row mx-0 p-0">
            ...
            <div id="chart" class="col-12 my-4 p-0">
                <div class="w-100">
                    ...
                    <div class="card --main">
                        ...
                        <div class="card-body">
                            <div class="row card__users mx-0">
                                @foreach (App\Models\User::where('role_id', '>', 3)->where('id', '!=', Auth::id())->orderBy('id', 'DESC')->limit(8)->get() as $user)
                    ...
                    <div class="card">
                                ...
                                @foreach ($tasks as $task)
                                    <div
                                        ...
                                        <div class="va-checkbox">
                                            <input type="checkbox" name="" id="tk-{{ $task->id }}"
                                                class="task__item--check" data-id="{{ $task->id }}"
                                                @if ($task->done == 1) checked @endif>
                                            <label for="tk-{{ $task->id }}">{{ $task->name }} @if ($task->done == 1)
                                                    <span class="badge badge-success">Hoàn Thành</span>
                                                @else
                                                    @if ($task->time_end > $now)
                                                        <span class="badge badge-warning">Còn
                                                            {{ timeDiff($now, $task->time_end) }}</span>
                                                    @else
                        ...
                        <div class="card-footer">
                            <div class="d-flex justify-content-center align-items-center">
                                <a href="" class="see-all btn__add--task" data-toggle="modal"
                                    data-target="#add__task"><i class="fas fa-plus pr-2"></i>Thêm Task</a>
                                <a href="" class="see-all btn_reload--task ml-4"><i
