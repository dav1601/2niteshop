@foreach ($categories as $category)
    @if ($category->id != 120)
        <li class="hm__item lv-{{ $category->level }}" data-lv={{ $category->level }}>
            ...
            <a
                href="{{ url('category/' . App\Models\Category::createUriCategory(App\Models\Category::ParentTree($category->id))) }}">
                <div class="icon-name">
    ...
    @if ($category->id == 112)
        ...
        @foreach ($categories_120 as $category_120)
            <li class="hm__item lv-{{ $category_120->level }}" data-lv={{ $category_120->level }}>
                <a
                    href="{{ url('category/' . App\Models\Category::createUriCategory(App\Models\Category::ParentTree(120))) }}">
                    <div class="icon-name">
                        <img src="{{ $file->ver_img_local('admin/images/category/icon/icons8-nintendo-switch-logo-64.png') }}"
                            width="25" height="25" alt="{{ $category_120->name }}">
