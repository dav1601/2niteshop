<div class="card-form @if ($modal) fix-auth-card @endif">
    <form method="POST" @if (!$modal) action="{{ route('navi_login') }}" @endif
        id="{{ !$modal ? '' : 'formLoginModal' }}">
        @csrf
        @if (!$modal)
            <h2 class="title"> Đăng Nhập</h2>
        @endif
        <p class="subtitle mb-2">Bạn chưa có tài khoản? <a class="@if ($modal) loadTempReg @endif"
                href="{{ $modal ? '#' : route('register') }}">Đăng Ký</a></p>
        @if (!$modal)
            <p class="subtitle mb-2">Quay về Trang Chủ? <a href="{{ route('home') }}">Trang Chủ</a></p>
        @endif
        <div class="social-login">
            <a href="{{ route('login_google') }}" class="btn__login btn__login--google">
                <img alt="Google"
                    src="data:image/svg+xml;base64," />
                <p class="btn-text">Đăng nhập bằng Google</p>
            </a>
            <a href="{{ route('login_fb') }}" class="btn__login btn__login--fb">
                <img alt="FB"
                    src="data:image/svg+xml;base64," />
                <p class="btn-text">Đăng nhập bằng Facebook</p>
            </a>
            <a href="{{ route('login_git') }}" class="btn__login btn__login--git">
                <img src="{{ asset('client/images/github.png') }}" alt="">
                <p class="btn-text">Đăng nhập bằng Github</p>
            </a>
        </div>

        <p class="or"><span>or</span></p>
        @if ($modal)
            <input type="hidden" name="ajax" value="true">
        @endif
        <div class="email-login">
            <div class="form-group">
                <label for="email"> <b>Email</b></label>
                <input type="email" class="input-auth-email" id="login-email" placeholder="Nhập Email" name="email"
                    value="{{ old('email') }}" required>
                @if (!$modal)
                    @error('email')
                        <span class="invalid-feedback">
                            <strong>{{ $message }}</strong>
                        </span>
                    @enderror
                @endif
            </div>
            <div class="form-group">
                <label for="psw"><b>Mật Khẩu</b></label>
                <input type="password" class="input-auth-pass" id="login-pass" placeholder="Nhập Mật Khẩu"
                    name="password" required>
                <i class="fa-solid fa-eye eyes-pass"></i>
            </div>
            @if (!$modal)
                @error('password')
                    <span class="invalid-feedback">
                        <strong>{{ $message }}</strong>
                    </span>
                @enderror
            @endif
        </div>
        <div class="wp-remember" style="display: flex; margin:10px 0; align-items:center">
            <input class="form-check-input" type="checkbox" name="remember" id="remember">
            <label class="form-check-label" style="padding-left: 5px" for="remember">
                {{ __('Ghi nhớ') }}
            </label>

        </div>
        <button type="submit" class="cta-btn">
            Đăng nhập
        </button>
        <p class="subtitle">Bạn quên mật khẩu? <a href="{{ route('password.request') }}">Khôi phục</a>
        </p>
    </form>

</div>
