<div class="card-form reg @if ($isModal) fix-auth-card @endif">
    <form method="POST" @if (!$isModal) action="{{ route('register') }}" @endif
        id="{{ $isModal ? 'formRegModal' : '' }}">
        @csrf
        @if ($isModal)
            <input type="hidden" name="ajax" value="true">
        @endif
        @if (!$isModal)
            <h2 class="title"> Đăng Ký</h2>
        @endif
        <p class="subtitle mb-2">Bạn đã có tài khoản? <a class="@if ($isModal) loadTempLogin @endif"
                href="{{ $isModal ? '#' : route('login') }}">Đăng Nhập</a></p>
        @if (!$isModal)
            <p class="subtitle mb-2">Quay về Trang Chủ? <a href="{{ $isModal ? '' : route('home') }}">Trang Chủ</a></p>
        @endif
        <div class="social-login">
            <a href="{{ route('login_google') }}" class="btn__login btn__login--google">
                <img alt="Google"
                    src="data:image/svg+xml;base64," />
                <p class="btn-text">Đăng nhập bằng Google</p>
            </a>
            <a href="{{ route('login_fb') }}" class="btn__login btn__login--fb">
                <img alt="FB"
                    src="data:image/svg+xml;base64," />
                <p class="btn-text">Đăng nhập bằng Facebook</p>
            </a>
            <a href="{{ route('login_git') }}" class="btn__login btn__login--git">
                <img src="{{ asset('client/images/github.png') }}" alt="">
                <p class="btn-text">Đăng nhập bằng Github</p>
            </a>
        </div>

        <p class="or"><span>or</span></p>

        <div class="email-login">
            <div class="form-group">
                <label for="name"> <b>Tên khách hàng</b></label>
                <input type="text" class="input-auth-text" placeholder="Nhập Họ và Tên" name="name"
                    value="{{ old('name') }}" id="reg-name" required>
                @if (!$isModal)
                    @error('name')
                        <span class="invalid-feedback">
                            <strong>{{ $message }}</strong>
                        </span>
                    @enderror
                @endif

            </div>
            <div class="form-group">
                <label for="email"> <b>Email</b></label>
                <input type="email" class="input-auth-email" placeholder="Nhập Email" name="email"
                    value="{{ old('email') }}" id="reg-email" required>
                @if (!$isModal)
                    @error('email')
                        <span class="invalid-feedback">
                            <strong>{{ $message }}</strong>
                        </span>
                    @enderror
                @endif
            </div>
            <div class="form-group">
                <label for="password"><b>Mật Khẩu</b></label>
                <input type="password" class="input-auth-pass" placeholder="Nhập Mật Khẩu" name="password" required
                    autocomplete="new-password" id="reg-pass">
                <i class="fa-solid fa-eye eyes-pass"></i>
                @if (!$isModal)
                    @error('password')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    @enderror
                @endif
            </div>
            <div class="form-group">
                <label for="password-confirm"><b>Nhập lại mật khẩU</b></label>
                <input type="password" placeholder="Nhập lại Mật Khẩu" name="password_confirmation" required
                    autocomplete="new-password" class="input-auth-pass" id="reg-repass">
                <i class="fa-solid fa-eye eyes-pass"></i>
                @if (!$isModal)
                    @error('password')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    @enderror
                @endif
            </div>
            <div class="form-group">
                <label for="phone"> <b>Số điện thoại khách hàng</b></label>
                <input type="text" class="input-auth-text" placeholder="Nhập Số điện thoại" name="phone"
                    value="{{ old('phone') }}" required id="reg-phone">
                @if (!$isModal)
                    @error('phone')
                        <span class="invalid-feedback">
                            <strong>{{ $message }}</strong>
                        </span>
                    @enderror
                @endif
            </div>

        </div>
        <input type="submit" value="Đăng Ký" class="cta-btn">
    </form>
</div>
