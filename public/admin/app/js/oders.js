$(function () {
    ...
    var load_data =  function load_data($action = "load", $type = 1, $sort = $(prefix + "sort").val(), $stt = $(prefix + "stt").val(), $nameOrMail = $(prefix + "nameMail").val(), $phone = $(prefix + "phone").val(), $dateP = $("#dateprev input").val(), $dateN = $("#datenext input").val(), $p = $(prefix + "prov").val(), $d = $(prefix + "dist").val(), $w = $(prefix + "ward").val(), $id = 0, $page = 1, $val = 0) {
        var url = $(prefix + "url").val();
        $.ajax({
            type: "post",
            url: url,
            data: { act: $action, type: $type, sort: $sort, stt: $stt, nameOrMail: $nameOrMail, phone: $phone, dateP: $dateP, dateN: $dateN, p: $p, d: $d, w: $w, id: $id, page: $page, val: $val },
            dataType: "json",
            success: function (data) {
                ...
                if (data.type == 3) {
                    var mess = "Cập nhật TT thanh toán đơn hàng số " + $id + " thành công";
    ...
    $(document).on('change', '.update__status', function () {
        ...
        load_data($action = "update_stt", $type = 2, $sort = $(prefix + "sort").val(), $stt = $(prefix + "stt").val(), $nameOrMail = $(prefix + "nameMail").val(), $phone = $(prefix + "phone").val(), $dateP = $("#dateprev input").val(), $dateN = $("#datenext input").val(), $p = $(prefix + "prov").val(), $d = $(prefix + "dist").val(), $w = $(prefix + "ward").val(), $id = id, $page = currentPage, $val = val);
    });
    $(document).on('change', '.update__paid', function () {
        var id = $(this).attr('data-id');
        var val = $(this).val();
        load_data($action = "update_paid", $type = 3, $sort = $(prefix + "sort").val(), $stt = $(prefix + "stt").val(), $nameOrMail = $(prefix + "nameMail").val(), $phone = $(prefix + "phone").val(), $dateP = $("#dateprev input").val(), $dateN = $("#datenext input").val(), $p = $(prefix + "prov").val(), $d = $(prefix + "dist").val(), $w = $(prefix + "ward").val(), $id = id, $page = currentPage, $val = val);
    });
    $(document).on('click', ".page-link", function () {
        load_data($action = "load", $type = 1, $sort = $(prefix + "sort").val(), $stt = $(prefix + "stt").val(), $nameOrMail = $(prefix + "nameMail").val(), $phone = $(prefix + "phone").val(), $dateP = $("#dateprev input").val(), $dateN = $("#datenext input").val(), $p = $(prefix + "prov").val(), $d = $(prefix + "dist").val(), $w = $(prefix + "ward").val(), $id = 0, $page = $(this).attr('data-page'), $val = 0);
    });
    ...
    var intervalCustomers = function load_data_customers($action = "load", $type = 1, $sort = $(prefix_2 + "sort").val(), $vip = $(prefix_2 + "vip").val(), $nameOrMail = $(prefix_2 + "nameMail").val(), $phone = $(prefix_2 + "phone").val(), $wF = $(prefix_2 + "wF").val(), $wT = $(prefix_2 + "wT").val(), $p = $(prefix_2 + "prov").val(), $d = $(prefix_2 + "dist").val(), $w = $(prefix_2 + "ward").val(), $page = 1) {
        var url_customers = $(prefix_2 + "urlAjax").val();
        $.ajax({
            type: "post",
            url: url_customers,
            data: { act: $action, vip: $vip, type: $type, sort: $sort, nameOrMail: $nameOrMail, phone: $phone, wF: $wF, wT: $wT, p: $p, d: $d, w: $w, page: $page },
            dataType: "json",
            success: function (data) {
