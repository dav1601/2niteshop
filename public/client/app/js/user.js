$(function () {
    ...
    function checkAddAddress() {
        ...
        if (name == "" || phone == "" || prov == 0 || dist == 0 || ward == 0 || detail == "") {
    ...
    $(document).on('click', "#add__address", function () {
        ...
        $.ajax({
            ...
            data: { name: name, phone: phone, prov: prov, prov_id: prov_id, dist: dist, dist_id: dist_id, ward: ward, ward_id: ward_id, detail: detail, type: type, def: def, act: act },
            dataType: "json",
            beforeSend: function () {
                $.loading();
            },
    ...
    $(document).on('click', "#update__address", function () {
        ...
        $.ajax({
            type: "post",
            url: route('ajax__address'),
            data: { name: name, phone: phone, prov: prov, prov_id: prov_id, dist: dist, dist_id: dist_id, ward: ward, ward_id: ward_id, detail: detail, type: type, def: def, act: act, id: id },
            dataType: "json",
            beforeSend: function () {
                $.loading();
            },
    ...
    $(document).on('click', ".aa__btn", function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.end_loading();
                if (data.ok == 1) {
                    renderAlert("success", "Cập nhật địa chỉ mặc định Thành công");
                } else {
    ...
    $(document).on('change', "#prov", function () {
        ...
        if ($(this).val() == 0) {
            ...
            $("#ward").html('<option value="0">Bạn Chưa Chọn Quận/Huyện</option>');
        } else {
            $.ajax({
                type: "post",
                url: url,
