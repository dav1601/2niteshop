$(function () {
    ...
    function setHeight() {
        ...
        if ($(".home__left").css('display') != "none") {
            ...
            $("#biad__content--home .home__right--slide #homeSlide img").css('height', heightHome);
            var heightSlide = $("#biad__content--home .home__right--slide #homeSlide img").height();
            $(".home__right--banner img").css('height', parseInt(heightSlide / 4 - 8));
        } else {
            var heightSlide = $("#biad__content--home .home__right--slide #homeSlide").height();
            $(".home__right").css('height', 'auto');
            $("#biad__content--home .home__right--slide #homeSlide img").css('height', 'auto');
            var heightSlide = $("#biad__content--home .home__right--slide #homeSlide img").height();
            $(".home__right--banner img").css('height', parseInt(heightSlide / 4 - 5));
        }
    }
    setHeight();
    $(window).resize(function () {
    ...
    function checkScrollTop() {
        ...
        if (top >= scroll + 10) {
            $("#header__scroll").addClass('davi__sticky animate__animated animate__slideInDown').removeClass('d-none animate__slideInUp');
            if (nameRoute == "home") {
                $(".bot__left").addClass('drop__show');
            }
            $(".home__left").css('visibility', 'hidden');
        } else {
            $("#header__scroll").removeClass('davi__sticky animate__animated  animate__slideInDown').addClass('animate__slideInUp d-none');
    ...
    function settingBtnModal() {
        var widthModal = parseInt($('.modal-content').width()) - 12;
        var heightModal = $('.modal-content').height();
        $('.box__sub--btn').css('margin-top', '523px');
    ...
    $('#quickModal').on('shown.bs.modal	', function (event) {
        ...
        function toggleControls() {
            if (sub_slide.getCurrentSlideCount() == sub_slide.getTotalSlideCount()) {
    ...
    // //////////////////////////////////
    $(document).on('mouseenter', '.product__item', function (event) {
        var id = $(this).attr('data-id');
        $(".qv__" + id).addClass('animate__animated animate__zoomInDown');
    }).on('mouseleave', '.product__item', function () {
        $(".quick__view").removeClass('animate__animated  animate__zoomInDown');
    });
    // ///////////////
    $(document).on('mouseenter', '.product__item--list', function (event) {
        var id = $(this).attr('data-id');
        $(".qv__" + id).addClass('animate__animated animate__zoomInDown');
    ...
    $(document).on('click', ".btn-cart", function () {
        ...
        $.ajax({
            type: "post",
            url: url_cart,
            data: { id: id, qty: qty, sub_total: sub_total, op: op, type: "add" },
            dataType: "json",
            beforeSend: function () {
                $.loading();
            },
    ...
    $(document).on('click', "#preOrder", function () {
        $("#modal__body--preOrd #preOrderNow").attr('data-id', $(this).attr('data-id'));
    ...
    $(document).on('click', "#preOrderNow", function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.created == 1) {
                    renderAlert("success", "Quý khách đã gửi thông tin đặt hàng thành công.Bộ phận hỗ trợ sẽ liên lạc sớm nhất với quý khách.");
                    $('#preOrdModal').modal('hide');
                }
                if (data.created == 0) {
                    renderAlert("error", "Gửi thông tin đặt hàng thất bại. Quý khách thông cảm đặt hàng lại thành thật xin lỗi quý khách.");
                }
            }
        });
    });
    //  /////////////////////////////
    $('.hm__item').hover(function () {
        $(this).has('.sub__menu').children('.sub__menu').css('display', 'block');
    }, function () {
        $(this).has('.sub__menu').children('.sub__menu').css('display', 'none');
    }
    );
    ...
    $('.home__right--slide').hover(function () {
        $('.slide__btn').css('visibility', 'unset');
        $('.slide__btn.--next').removeClass('animate__animated animate__fadeOutRight').addClass('animate__animated animate__fadeInRight');
        $('.slide__btn.--prev').removeClass('animate__animated animate__fadeOutLeft').addClass('animate__animated animate__fadeInLeft');
    }, function () {
        $('.slide__btn.--next').removeClass('animate__animated animate__fadeInRight').addClass('animate__animated animate__fadeOutRight');
        $('.slide__btn.--prev').removeClass('animate__animated animate__fadeInLeft').addClass('animate__animated animate__fadeOutLeft');
    ...
    $(document).on('click', ".icon-toggle", function () {
        ...
        if ($(this).children('i').hasClass('fa-circle-plus') && !$("#collapse-" + id).hasClass('show')) {
